    ...
    >
      <SidebarHeader className="h-14">
        <div className="flex items-center justify-between px-2 pt-1">
          <SidebarMenuButton size="lg" className="hover:bg-transparent px-2 transition-all active:scale-95">
            <div className="flex items-center gap-2">
              <div className="flex size-8 items-center justify-center bg-indigo-600 rounded-full transition-transform hover:scale-110">
              ...
              <SidebarGroupContent className="mt-1.5">
                <SidebarMenu className="px-2 gap-0.5">
                  {data.insightAndControl.map((item) => (
                    <SidebarMenuItem key={item.title}>
                      <SidebarMenuButton className="h-9 px-2 rounded-lg text-neutral-500 dark:text-neutral-300 font-medium text-[13px] hover:bg-neutral-200/60 dark:hover:bg-neutral-800/60 transition-all active:scale-[0.98]">
                        <item.icon className="size-[1.1rem]!" />
                        <span>{item.title}</span>
                      </SidebarMenuButton>
                    </SidebarMenuItem>
              ...
              <SidebarGroupContent className="mt-1.5">
                <SidebarMenu className="px-1 gap-0.5">
                  {data.favorites.map((item) => (
                    <SidebarMenuItem key={item.title}>
                      <SidebarMenuButton className="h-9 px-2 rounded-lg text-neutral-500 dark:text-neutral-300 font-medium text-[13px] hover:bg-neutral-200/60 dark:hover:bg-neutral-800/60 transition-all active:scale-[0.98]">
                        <div className={`size-5 rounded-sm flex items-center justify-center text-[10px] text-white font-semibold shadow-sm ${item.color} transition-all`}>
                          {item.title[0]}
                        </div>
                        <span>{item.title}</span>
                      </SidebarMenuButton>
                  ...
                  <SidebarMenuItem>
                    <SidebarMenuButton className="h-9 px-2 text-neutral-500 font-medium text-[13px] hover:bg-neutral-200/40 dark:hover:bg-neutral-800/40 transition-all active:scale-95">
        ...
        <div className="rounded-2xl bg-white dark:bg-neutral-900 p-4 flex flex-col gap-4 border border-neutral-200 dark:border-neutral-800 shadow">
          ...
          <Dialog>
            <DialogTrigger asChild>
              <Button className="h-10 flex items-center justify-center gap-2 tracking-tight bg-neutral-100 dark:bg-neutral-800 py-2.5 rounded-xl text-[14px] text-neutral-500 dark:text-neutral-300 font-medium border border-neutral-200 dark:border-neutral-700 hover:bg-neutral-200 dark:hover:bg-neutral-800 transition-all active:scale-95 shadow-sm hover:shadow-md">
                <IconSparkles className="size-4 fill-indigo-600 dark:fill-indigo-400 transition-transform" />
                Upgrade to Pro
              </Button>
            </DialogTrigger>
            <DialogContent className="max-w-md">
              ...
              <DialogFooter className="flex-col sm:flex-col gap-2">
                <Button className="w-full bg-indigo-600 hover:bg-indigo-700 text-white dark:bg-indigo-500 dark:hover:bg-indigo-600 border-none rounded-xl h-11 transition-all active:scale-[0.98]">
                  Upgrade Now
                </Button>
