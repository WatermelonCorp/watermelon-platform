...
export const StreamCard = ({ card }: { card: StreamCardProps }) => (
    <Card className="shadow-none bg-neutral-200/35 dark:bg-neutral-900 rounded-lg border-none px-3 py-2.5 gap-3 transition-colors duration-300 hover:bg-muted/50 cursor-pointer group">
        <CardHeader className="p-0 flex flex-row items-start justify-between space-y-0">
            ...
            <Avatar className="size-8 border-2 border-white dark:border-neutral-800 shadow-sm transition-colors duration-300 hover:ring-2 hover:ring-indigo-500/30 cursor-pointer">
                <AvatarImage src={card.avatar} className="grayscale hover:grayscale-0 transition-colors duration-300" />
                <AvatarFallback>
            ...
            <div className="flex flex-wrap gap-2 mt-1">
                {card.tags.map((tag, i) => (
                    <Badge
                        ...
                        variant="secondary"
                        className={cn(
                            "flex items-center gap-1.5 px-2 py-1 rounded-full shadow-none text-[12px] tracking-tight transition-colors duration-300 hover:shadow-sm cursor-pointer border dark:border-neutral-700/50",
                            tag.bg,
                            tag.color,
